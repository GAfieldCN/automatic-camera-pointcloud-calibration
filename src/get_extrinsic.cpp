...
string lidar_path, photo_path, intrinsic_path, extrinsic_path, output_path;
int error_threshold;
vector<float> init;
...
void getParameters() {
    ...
    if (!ros::param::get("output_error_path", output_path)) {
        cout << "Can not get the value of output path" << endl;
        exit(1);
    }
...
int main(int argc, char **argv) {
    ...
    cout << " " << endl;
    cout << "the extrinsic is saved in data/parameters" << endl;
    cout << "Use the extrinsic result to reproject the data" << endl;
    float error[2] = {0, 0};

    std::ofstream out(output_path);
    getUVError(intrinsic_path, extrinsic_path, lidar_path, photo_path, error, error_threshold, out);
    
    cout << "u average error is: " << error[0] << endl;
    cout << "v average error is: " << error[1] << endl;
    cout << "the whole reprojection error is saved in data/parameters" << endl;
